...
def run(dataset, option):
    ...
    if option.depthNet == 0:
        ...
        checkpoint = torch.load(leres_model_path, map_location=str(device))
        leresmodel = RelDepthModel(backbone='resnext101')
        leresmodel.load_state_dict(
    ...
    for image_ind, images in enumerate(dataset):
        ...
        whole_image_optimal_size, patch_scale = calculateprocessingres(
            img, option.net_receptive_field_size, r_threshold_value, scale_threshold,
        ...
        whole_estimate = doubleestimate(
            img, option.net_receptive_field_size, whole_image_optimal_size, option.pix2pixsize,
            option.depthNet
        )
        if option.R0 or option.R20:
            path = os.path.join(result_dir, images.name)
            if option.output_resolution == 1:
                midas.utils.write_depth(
                    path,
                    cv2.resize(
                        whole_estimate, (input_resolution[1], input_resolution[0]),
                ...
                midas.utils.write_depth(
                    path, whole_estimate, bits=2, colored=option.colorize_results
                )
            continue
        ...
        for patch_ind in range(len(imageandpatchs)):
            ...
            patch_estimation = doubleestimate(
                patch_rgb, option.net_receptive_field_size, option.patch_netsize,
            ...
            patch_whole_estimate_base = cv2.resize(
                patch_whole_estimate_base, (option.pix2pixsize, option.pix2pixsize),
                interpolation=cv2.INTER_CUBIC
            )
            ...
            tobemergedto[h1:h2, w1:w2] = np.multiply(tobemergedto[h1:h2, w1:w2],
                                                     1 - mask) + np.multiply(merged, mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='result dir. result depth will be png.'
    ...
    parser.add_argument(
        '--net_receptive_field_size', type=int, required=False
